...
export function DeepQualityScanClient() {
  ...
  return <section className="card section-card"><div className="section-heading"><div><h3>深度数据质量检查</h3><p className="notes">按需读取单元格存储类型、公式和值；不会修改历史数据。</p></div><button className="primary-button" onClick={run} disabled={loading}>{loading ? '扫描中…' : '运行深度检查'}</button></div>{!result && !error && <div className="empty-state">状态：尚未运行</div>}{error && <div className="notice error">{error}</div>}{result && <><p className="notes">状态：{result.status} · 最后扫描：{result.scannedAt} · 扫描行：{result.scannedRows} · 异常：{result.issueCount}</p><p className="notes">实际执行：{result.ruleCoverage.map((rule) => rule.code).join(' · ')}</p>{result.exceptions.length === 0 ? <div className="empty-state">本次深度检查未发现异常</div> : <div className="table-wrap"><table><thead><tr><th>Severity</th><th>Code</th><th>Row</th><th>Description</th><th>Suggested action</th></tr></thead><tbody>{result.exceptions.map((item, index) => <tr key={`${item.code}-${item.ledgerRow ?? index}`}><td>{item.severity}</td><td>{item.code}</td><td>{item.ledgerRow ?? '—'}</td><td>{item.description}</td><td>{item.suggestedAction}</td></tr>)}</tbody></table></div>}</>}</section>;
}
