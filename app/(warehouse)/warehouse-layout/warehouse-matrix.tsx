...
export function WarehouseMatrix({ locations, selectableSku, requiredQty=1, selectedLocation, onSelect }: {
  ...
  return <div className={`warehouse-matrix${onSelect?' selection-mode':''}`}>
    <div className="matrix-legend"><span><i className="empty"/>空库位</span><span><i className="occupied"/>有库存</span>{onSelect&&<span><i className="eligible"/>可选来源</span>}<small>R1-2-3-L = 货架1 · 第2排 · 第3个 Bay · 左侧</small></div>
    {racks.map(rack=>{
      ...
      return <section className="matrix-rack" key={rack}>
        ...
        <tbody>{rows.map(row=><tr key={row}><th>第{row}排</th>{bays.flatMap(bay=>(['L','M','R'] as const).map(side=>{
          ...
          return <MatrixCell key={`${row}-${bay}-${side}`} item={entry?.item} selectableSku={selectableSku} requiredQty={requiredQty} selected={entry?.item.location===selectedLocation} onSelect={onSelect}/>;
        }))}</tr>)}</tbody></table></div>
      </section>;
    })}
    {services.length>0&&<section className="matrix-service"><h3>服务区域</h3><div className="matrix-service-scroll"><table><tbody><tr>{services.map(item=><MatrixCell key={item.location} item={item} selectableSku={selectableSku} requiredQty={requiredQty} selected={item.location===selectedLocation} onSelect={onSelect}/>)}</tr></tbody></table></div></section>}
  </div>;
}

function MatrixCell({item,selectableSku,requiredQty,selected,onSelect}:{item?:MatrixLocation | undefined;selectableSku?:string | undefined;requiredQty:number;selected:boolean;onSelect?: ((location:string)=>void) | undefined}) {
  const eligible=Boolean(item&&selectableSku&&item.skuLines.some(line=>line.sku===selectableSku&&line.qty>=requiredQty));
  ...
  return <td className={`matrix-cell ${occupied?'occupied':'empty'} ${eligible?'eligible':''} ${selected?'selected':''}`}>
    {onSelect&&eligible?<button type="button" onClick={()=>onSelect(item!.location)} aria-label={`选择库位 ${item!.location}`}>{content}</button>:<div>{content}</div>}
  </td>;
}
