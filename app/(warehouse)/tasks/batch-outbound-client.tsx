...
export function BatchOutboundClient({ tasks, businessDate }: { tasks: OperationalTask[]; businessDate: string }) {
  ...
  async function submit(mode: 'preview' | 'confirm') {
    ...
    try {
      const response = await fetch('/api/warehouse/outbound/batch', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ mode, date: businessDate, outboundDate, items }) });
