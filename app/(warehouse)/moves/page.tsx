...
export default async function Page() {
  ...
  const locations = process.env.WAREHOUSE_DESIGN_FIXTURE==='true'?warehouseDesignFixture():(await warehouseReadAdapterFromEnv().readLocationSummaries()).locations;
  return <><header className="console-header"><div><h2>库存作业 <small>扫码批量处理</small></h2><p>先导入 SN，再从仓库图选择目标库位；系统逐台校验后统一确认。</p></div><div className="write-live">READ → VALIDATE → PREVIEW → CONFIRM → WRITE → VERIFY</div></header><OperationsClient businessDate={todayInSydney()} canAdjust={canAdjust} locations={locations}/></>;
}
