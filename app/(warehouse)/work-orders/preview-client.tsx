...
export function WorkOrderPreviewClient({ initialBusinessDate, locations }: { initialBusinessDate: string; locations: MatrixLocation[] }) {
  ...
  return <><div className="operations-console">
    ...
    <aside className="detail-panel prepared-detail"><h3>备货标签与现场确认</h3>{labels.length>0&&<div className="detail-section label-stage"><h4>1 · 先打印取货标签</h4><p>按 Pickup Code 生成；打印不会写入台账。</p><LabelSelector labels={labels} selected={selectedLabels} setSelected={setSelectedLabels}/><button onClick={()=>window.print()} className="locked-action ready" disabled={!selectedLabels.size}><Printer size={18}/>打印已选 {selectedLabels.size} 张标签</button></div>}{!active?<div className="detail-empty">选择可用工单后，打印标签并进行现场取货。</div>:<><div className="detail-section"><h4>2 · 找货后扫描并确认</h4></div><dl><dt>SH</dt><dd>{active.document.sh??'—'}</dd><dt>SKU</dt><dd>{activeRow?.sku??'—'}</dd><dt>ERP 仓库</dt><dd>{activeRow?.erpWarehouse??'—'}</dd><dt>库存属性</dt><dd>{activeRow?.stockCondition??'—'}</dd><dt>数量</dt><dd>{activeRow?.qty??'—'}</dd><dt>Pickup Code</dt><dd>{active.item.preview.pickupCode?.value??'—'}</dd><dt>系统建议库位</dt><dd>{active.item.preview.recommendation?.location??'无可用库位'}</dd></dl>
