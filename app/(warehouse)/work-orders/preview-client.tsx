...
export function WorkOrderPreviewClient({ initialBusinessDate, locations }: { initialBusinessDate: string; locations: MatrixLocation[] }) {
  ...
  const [labels, setLabels] = useState<PreparedPrintLabel[]>([]);
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true); setSystemError(undefined); setPreview(undefined); setDrafts({}); setLabels([]); setConfirmResult(undefined);
    try {
  ...
  async function confirmBatch(){
    ...
    try{
      const workOrders=preview.documents.map((document,documentIndex)=>({sh:document.sh!,sourceFileName:document.sourceFileName,
      ...
      if(!payload.ok) throw new Error(`${payload.error.code} · ${payload.error.message}`);
      setLabels(payload.data.labels); setSelectedLabels(new Set(payload.data.labels.map(item=>item.pickupCode)));
  ...
  return <><div className="operations-console">
    ...
    <aside className="detail-panel prepared-detail"><h3>{labels.length?'标签打印':'备货确认'}</h3>{labels.length?<LabelSelector labels={labels} selected={selectedLabels} setSelected={setSelectedLabels}/>:!active?<div className="detail-empty">选择一行，填写 SN 和最终来源库位。</div>:<><dl><dt>SH</dt><dd>{active.document.sh??'—'}</dd><dt>SKU</dt><dd>{activeRow?.sku??'—'}</dd><dt>ERP 仓库</dt><dd>{activeRow?.erpWarehouse??'—'}</dd><dt>库存属性</dt><dd>{activeRow?.stockCondition??'—'}</dd><dt>数量</dt><dd>{activeRow?.qty??'—'}</dd><dt>Pickup Code</dt><dd>确认时逐单生成</dd><dt>系统建议库位</dt><dd>{active.item.preview.recommendation?.location??'无可用库位'}</dd></dl>
      ...
      {labels.length?<button onClick={()=>window.print()} className="locked-action ready" disabled={!selectedLabels.size}><Printer size={18}/>打印已选 {selectedLabels.size} 张标签</button>:<button onClick={confirmBatch} className={`locked-action ${batchReady?'ready':''}`} disabled={!batchReady||confirming}>{confirming?'逐单写入并复核中…':batchReady?'确认全部工单并生成标签':'完成全部 SN 与库位确认'}</button>}
    </aside>
...
function LabelSelector({labels,selected,setSelected}:{labels:PreparedPrintLabel[];selected:Set<string>;setSelected:(value:Set<string>)=>void}){return <div className="label-selector"><div className="label-select-actions"><button onClick={()=>setSelected(new Set(labels.map(item=>item.pickupCode)))}>全选</button><button onClick={()=>setSelected(new Set())}>清空</button></div>{labels.map(label=><label key={label.pickupCode}><input type="checkbox" checked={selected.has(label.pickupCode)} onChange={()=>{const next=new Set(selected);next.has(label.pickupCode)?next.delete(label.pickupCode):next.add(label.pickupCode);setSelected(next);}}/><span><strong>{label.pickupCode}</strong><small>{label.sh} · {label.lines.reduce((sum,line)=>sum+line.qty,0)} 台</small></span></label>)}</div>;}
