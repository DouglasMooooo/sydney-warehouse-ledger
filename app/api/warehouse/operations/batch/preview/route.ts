...
import { previewControlledBatchTransfer, type ControlledBatchTransferInput } from '../../../../../../src/application/controlledLedgerOperation';
...
export async function POST(request: Request) {
  try {
    ...
    const auth = authenticateWarehouseRequest(request, 'MOVE_PREVIEW');
    expensiveOperationLimiter.check(`batch-transfer-preview:${auth.user.userId}`, 10, 60_000);
    return NextResponse.json(apiSuccess(await previewControlledBatchTransfer(body, warehouseReadAdapterFromEnv())));
  } catch (error) {
    const safe = clientSafeError(error);
    console.error('Batch transfer preview failed', serverSafeErrorSummary(error));
