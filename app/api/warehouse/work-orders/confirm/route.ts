...
import { confirmPreparedWorkOrder,confirmPreparedWorkOrderBatch,type PreparedBatchConfirmInput,type PreparedConfirmInput } from '../../../../../src/application/confirmPreparedWorkOrder';
...
export async function POST(request:Request){try{const auth=authenticateWarehouseRequest(request,'WORK_ORDER_CONFIRM');expensiveOperationLimiter.check(`prepared:${auth.user.userId}`,5,60_000);const body=await request.json() as PreparedConfirmInput|PreparedBatchConfirmInput;const adapter=warehouseReadAdapterFromEnv();const writer=openApiLedgerWriterFromEnv();const context={createdBy:auth.user.userId};const result='workOrders' in body?await confirmPreparedWorkOrderBatch(body,adapter,writer,process.env,context):await confirmPreparedWorkOrder(body,adapter,writer,process.env,context);return NextResponse.json(apiSuccess(result));}catch(error){const safe=clientSafeError(error);console.error('Prepared confirmation failed',serverSafeErrorSummary(error));const status=safe.code==='AUTHENTICATION_REQUIRED'?401:safe.code==='PERMISSION_DENIED'?403:safe.code==='RATE_LIMITED'?429:400;return NextResponse.json(apiFailure(safe.code,safe.message,safe.field),{status});}}
