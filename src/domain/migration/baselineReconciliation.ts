...
import type { BaselineReplaySimulationInput, BaselineReplaySimulationReport, MigrationBaselineCandidate, MigrationManifest } from './types.js';

export class BaselineReplaySimulation {
  simulate(input:BaselineReplaySimulationInput):BaselineReplaySimulationReport{
    const baselineMovements=input.candidates.filter(usableCandidate).map((item,index)=>baselineMovement(item,index));
    ...
    const all=[...preCutover,...baselineMovements,...postCutover],grouped=new Map<string,InventoryMovement[]>();
    for(const movement of all){if(!movement.sn)continue;const sn=canonicalizeSn(movement.sn);grouped.set(sn,[...(grouped.get(sn)??[]),movement]);}
    ...
    const rawTarget=targetProjection(input.physicalTarget),validTarget=rawTarget.filter(item=>item.valid).map(item=>item.projection),rawReconciliation=reconcileSnProjection(replays,rawTarget.map(item=>item.projection)),validReconciliation=reconcileSnProjection(replays,validTarget);
    return {baselineCandidates:input.candidates.length,verifiedCandidates:input.candidates.filter(item=>item.verificationStatus==='VERIFIED').length,reviewRequired:input.candidates.filter(item=>item.verificationStatus==='REVIEW_REQUIRED').length,candidateConflicts:input.candidates.filter(item=>item.verificationStatus==='CONFLICT').length,
      postCutoverMovements:postCutover.length,replayInStock:rawReconciliation.replayInStockCount,matched:rawReconciliation.matched,rawPhysicalCount:rawTarget.length,validPhysicalCount:validTarget.length,rawMatchRate:rate(rawReconciliation.matched,rawTarget.length),validEvidenceOnlyMatchRate:rate(validReconciliation.matched,validTarget.length),replayConflicts:rawReconciliation.replayConflicts.length,orderingRiskMovementCount:input.movements.filter(item=>item.businessDate===input.policy.cutoverDate).length,historicalEvidenceCount:preCutover.length};
  }
}
