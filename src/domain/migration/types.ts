...
export interface BaselineEvidence {source:BaselineEvidenceSource;field:'SN'|'SKU'|'DISPLAY_NAME'|'LOCATION'|'STOCK_CONDITION'|'CURRENT_STATE';value:string;confidence:Exclude<BaselineConfidence,'CONFLICT'>}
...
export interface MigrationBaselineCandidate extends Omit<MigrationBaselineRecord,'stockCondition'>{stockCondition:StockCondition|string;confidence:BaselineConfidence;candidateStatus:BaselineCandidateStatus;physicalEvidenceQuality:PhysicalEvidenceQuality;candidateOnly:true}
...
export interface BaselineReplaySimulationReport {baselineCandidates:number;verifiedCandidates:number;reviewRequired:number;candidateConflicts:number;postCutoverMovements:number;replayInStock:number;matched:number;rawPhysicalCount:number;validPhysicalCount:number;rawMatchRate:number;validEvidenceOnlyMatchRate:number;replayConflicts:number;orderingRiskMovementCount:number;historicalEvidenceCount:number}
