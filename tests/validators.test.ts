...
test('Return to Repair allows missing SKU but requires SN', () => {
  const valid = validateLedgerInput({ action: '退回维修', date: TEST_DATE, sn: 'SN1', qty: 1, toLocation: 'RETURN-01', stockCondition: '待修' });
  assert.equal(valid.ok, true);
  const invalid = validateLedgerInput({ action: '退回维修', date: TEST_DATE, qty: 1, toLocation: 'RETURN-01', stockCondition: '待修' });
  assert(invalid.errors.some((error) => error.code === 'MISSING_SN'));
});
test('Prepared missing source location is rejected', () => {
  const result = validateLedgerInput({ action: '备货', date: TEST_DATE, shNo: 'SH1', pickupCode: 'SYD-00315', sku: '001', qty: 1, erpWarehouse: 'Sydney', stockCondition: '新机' });
...
test('Prepared requires Outbound Date to remain blank', () => {
  const result = validateLedgerInput({
    action: '备货', date: TEST_DATE, outboundDate: TEST_DATE, shNo: 'SH1',
...
test('serialized decimal quantity is rejected', () => {
  const result = validateLedgerInput({ action: '退回维修', date: TEST_DATE, sn: 'SN1', qty: 1.5, toLocation: 'RETURN-01', stockCondition: '待修' });
...
test('Return to Repair rejects any condition other than 待修', () => {
  const result = validateLedgerInput({
    action: '退回维修', date: TEST_DATE, sn: 'SN1', qty: 1,
...
test('Move requires trusted source condition and cannot change it', () => {
  const missingSource = validateLedgerInput({
    action: '移库', date: TEST_DATE, sn: 'SN1', qty: 1,
...
for (const action of ['期初库存', '入库'] as const) {
  test(`${action} requires date, qty, target location, and stock condition`, () => {
    ...
    assert.equal(validateLedgerInput({
      action, date: TEST_DATE, qty: 1, toLocation: 'R1-1-1-L', stockCondition: '新机',
    }).ok, true);
  });
}
...
test('Outbound keeps strict normal work-order validation', () => {
  const result = validateLedgerInput({ action: '出库', date: TEST_DATE, qty: 3, stockCondition: '新机' });
