...
| Write serialization | `PROCESS_LOCAL_MUTEX`; valid only while UAT runs one application instance | PASS in code / verify Render has one instance |
...
`PROCESS_LOCAL_MUTEX IS NOT A DISTRIBUTED LOCK.` Do not horizontally scale the FEISHU_UAT service above one instance without a separately reviewed distributed lock or atomic persistence mechanism.
