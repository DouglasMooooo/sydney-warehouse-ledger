...
The application now derives stable `MOV-YYYYMMDD-xxxxxx` identities for future system-native movements and deterministic `LEGACY-<hash>` identities for historical projection. A durable system-native ID is still required before public Movement or SN Lifecycle APIs are enabled.
